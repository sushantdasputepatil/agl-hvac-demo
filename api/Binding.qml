...
WebSocket {
    ...
    property Connections c : Connections {
        target: root
        onFanSpeedChanged: {
            var json = [MessageId.call, '9999', 'demohvac/set', {'FanSpeed': fanSpeed}]
            console.debug(JSON.stringify(json))
            sendTextMessage(JSON.stringify(json))
        }
        onLeftTemperatureChanged: {
            var json = [MessageId.call, '9999', 'demohvac/set', {'LeftTemperature': leftTemperature}]
            console.debug(JSON.stringify(json))
            sendTextMessage(JSON.stringify(json))
        }
        onRightTemperatureChanged: {
            var json = [MessageId.call, '9999', 'demohvac/set', {'RightTemperature': rightTemperature}]
            console.debug(JSON.stringify(json))
            sendTextMessage(JSON.stringify(json))
        }
    }
